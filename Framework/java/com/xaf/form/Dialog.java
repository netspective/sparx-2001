...
public class Dialog
{
	...
	static public final String GENERATE_DC_ATTR_VALUE  = "generate";
    ...
    */
	...
	public static Class findDialogContextClass(String packageName, Element elem)
	{
		...
		try
		{
			if(dcClassName != null && dcClassName.length() > 0 && ! dcClassName.equals(GENERATE_DC_ATTR_VALUE))
			...
			else
			{
				...
				dcClassName = "dialog.context." + dlgName;
				try
				{
					dcClass = Class.forName(dcClassName);
				}
	...
	public String getSubclassedDialogContextCode()
	{
		...
		code.append("package dialog.context." + pkgName + ";\n\n");
		code.append("import com.xaf.form.*;\n\n");
		code.append("public class " + com.xaf.xml.XmlSource.xmlTextToJavaIdentifier(getNameFromXml(), true) + " extends DialogContext\n");
		code.append("{\n");
		...
		while(i.hasNext())
		{
			...
			code.append(mi.getGetterMethodCode());
			code.append(mi.getSetterMethodCode());
			code.append("\n");
		}
