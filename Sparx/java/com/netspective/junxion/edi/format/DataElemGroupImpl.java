...
public class DataElemGroupImpl extends DefaultHandler implements Unmarshallable, DataElemGroup {
    ...
    public static DataElemGroup unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static DataElemGroup unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static DataElemGroup unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static DataElemGroup unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static DataElemGroup unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static DataElemGroup unmarshal(Reader reader, boolean validate) throws IOException {
        DataElemGroupImpl dataElemGroup = new DataElemGroupImpl();
        dataElemGroup.setCurrentUNode(dataElemGroup);
        dataElemGroup.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            parser.setContentHandler(dataElemGroup);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("data-elem-group")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
