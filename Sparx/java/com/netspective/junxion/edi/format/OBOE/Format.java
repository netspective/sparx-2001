...
public class Format implements DataModel
{
    ...
    public TransactionSet createTransactionSet() throws DataModelException
    {
        if(transactionSet != null)
            throw new DataModelException("Only a single transaction set is allowed in any given Format");
    ...
    public static void main(String[] args)
    {
        ...
        DataModelSchema.ParseContext pc = DataModelSchema.parse(format, new File("c:/projects/sparx/web-shared/resources/edi/formats/OBOE/ansi/270.xml"));
        System.out.println(pc.getSyntaxErrors().toString());
        System.out.println(format.getTransactionSet().getName());
        System.out.println(format.getTransactionSet().getTables().size());

        Table table = (Table) format.getTransactionSet().getTables().get(0);
        System.out.println(table.getSection().getValue());
        System.out.println(table.getSegments().size());
    }
}
