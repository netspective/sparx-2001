...
/**
 * $Id: StatementTask.java,v 1.14 2003-03-05 23:37:48 aye.thu Exp $
 */
...
public class StatementTask extends BasicTask
{
    ...
    public void execute(TaskContext tc) throws TaskExecuteException
    {
        ...
        try
        {
            ...
            {
                StatementInfo si = stmtManager.getStatement(tc.getServletContext(), null, stmtName);
                if (si != null)
                {
                    StatementDialog stmtDialog = new StatementDialog(si, getReport(), getSkin() != null ? getSkin().getValue(tc) : null, null);
                    stmtDialog.setRowsPerPage(rowsPerPage);
                    if(stmtDialog != null)
                    {
                        DialogContext dc = stmtDialog.createContext(context, tc.getServlet(), (HttpServletRequest) tc.getRequest(),
                                (HttpServletResponse) tc.getResponse(), SkinFactory.getInstance().getDialogSkin());
                        stmtDialog.prepareContext(dc);
                        stmtDialog.renderHtml(out, dc, true);
                    }
                }
                else
                {
                    throw new TaskExecuteException("Statement '" + stmtName + "' does not exist");
                }
            }
