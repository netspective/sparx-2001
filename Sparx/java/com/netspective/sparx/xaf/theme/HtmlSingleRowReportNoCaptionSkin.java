...
/**
 * $Id: HtmlSingleRowReportNoCaptionSkin.java,v 1.1 2003-02-24 03:47:31 aye.thu Exp $
 */
    ...
    public void produceDataRows(Writer writer, ReportContext rc, Object[] rowData) throws IOException
    {
        ...
        if(horizontalLayout)
        {
            ...
            for(int i = 0; i < dataColsCount; i++)
            {
                ...
                dataTable.append("<td class=\"report-field\">" +
                        column.getHeading().getValue(rc) + "</font></td>");
                dataTable.append("<td class=\"report\" align='" + ALIGN_ATTRS[column.getAlignStyle()] + "'>" +
                        (state.flagIsSet(ReportColumn.COLFLAG_WRAPURL) ? "<a href='" + state.getUrl() + "'>" + data + "</a>" : data) + "</td>");
