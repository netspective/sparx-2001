	...
	<tr class="data_table">
		<td class="data_table">
			<a title="Click here to test this dialog">
				...
				<img border="0">
					<xsl:attribute name="src"><xsl:value-of select="$sparx.ace.images-root-url"/>/icons/exec_dialog.gif</xsl:attribute>
				</img>
			</a>
			<img border="0"><xsl:attribute name="src"><xsl:value-of select="$sparx.ace.images-root-url"/>/icons/spacer.gif</xsl:attribute></img>
			<a title="Click here to review functional specifications"><xsl:attribute name="href"><xsl:value-of select="concat($root-url,'/describe/',@qualified-name)"/></xsl:attribute><img border="0"><xsl:attribute name="src"><xsl:value-of select="$sparx.ace.images-root-url"/>/icons/describe_dialog.gif</xsl:attribute></img></a>
		</td>
...
<xsl:template name="field-detail">
	...
	<xsl:if test="not($no-separator)">
		<tr><td colspan="10"><img width="100%" height="1"><xsl:attribute name="src"><xsl:value-of select="$sparx.shared.images-url"/>/design/bar.gif</xsl:attribute></img></td></tr>
	</xsl:if>
...
<xsl:template match="dialog" mode="detail">
	...
	<tr class="heading">
		<td class="heading">
			<img border="0"><xsl:attribute name="src"><xsl:value-of select="$sparx.ace.images-root-url"/>/icons/describe_dialog.gif</xsl:attribute></img>
