	...
	<tr>
		<td class="data_table">
			<a title="Click here to see test statement">
				...
				<img border="0">
					<xsl:attribute name="src"><xsl:value-of select="$sparx.ace.images-root-url"/>/icons/exec_query_defn.gif</xsl:attribute>
				</img>
			</a>
			<img border="0"><xsl:attribute name="src"><xsl:value-of select="$sparx.ace.images-root-url"/>/icons/spacer.gif</xsl:attribute></img>
			<a title="Click here to review functional specifications"><xsl:attribute name="href"><xsl:value-of select="concat($root-url, '/', 'describe', '/', @id)"/></xsl:attribute><img border="0"><xsl:attribute name="src"><xsl:value-of select="$sparx.ace.images-root-url"/>/icons/describe_query_defn.gif</xsl:attribute></img></a>
		</td>
...
<xsl:template match="query-defn" mode="detail">
	...
	<tr class="heading">
		<td class="heading">
		<img border="0"><xsl:attribute name="src"><xsl:value-of select="$sparx.ace.images-root-url"/>/icons/describe_query_defn.gif</xsl:attribute></img>
	...
	<table class="data_table" cellspacing="0" cellpadding="2" border="0">
		...
		<xsl:for-each select="select-dialog">
			<tr class="data_table" valign="top">
			<td class="data_table">
				<a title="Click here to test dialog"><xsl:attribute name="href"><xsl:value-of select="concat($root-url, '/test/query-defn-dlg/', ../@id, '/', @name)"/></xsl:attribute><img border="0"><xsl:attribute name="src"><xsl:value-of select="$sparx.ace.images-root-url"/>/icons/exec_dialog.gif</xsl:attribute></img></a>
			</td>
