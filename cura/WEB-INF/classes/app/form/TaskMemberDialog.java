...
import app.dal.domain.row.TaskPersonRelationRow;
import app.dal.domain.row.ProjectPersonRelationRow;
import app.dal.domain.rows.TaskPersonRelationRows;
...
public class TaskMemberDialog   extends Dialog
{
    ...
    protected void processEditData(DialogContext dc)
    {
        try
        {
            ...
            cc.beginTransaction();
            app.form.context.task.PersonRegistrationContext rc = (app.form.context.task.PersonRegistrationContext) dc;
            // update the Task Person Relationship table
            TaskPersonRelationTable tprTable = app.dal.DataAccessLayer.instance.getTaskPersonRelationTable();
            ...
            if (tprRows == null || tprRows.size() == 0)
            {
                ...
                for (int i=0; i< personList.length; i++)
                {
                    ...
                    tpr.setRelType(app.dal.table.TaskPersonRelationTypeTable.EnumeratedItem.MEMBER);
