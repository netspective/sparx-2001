...
public class AppLoginDialog extends LoginDialog
{
	...
	public AuthenticatedUser createUserData(DialogContext dc)
	{
		...
		try
		{
			activeSql = "person.active-org-memberships";
			ri = stmtMgr.execute(dbc, dc, null, activeSql, new Object[] { personId });
		...
		{
			String extraDebugging = "\n" + "personId = " + personId + ", memberOrgs = " + memberOrgs;
			StatementInfo activeSqlSI = activeSql != null ? stmtMgr.getStatement(activeSql) : null;
			if(activeSqlSI != null)
				extraDebugging += "\n statement = '" + activeSqlSI.getId() + "' " + activeSqlSI.getSql(dc);
			if(ri != null)
				throw new RuntimeException(e.toString() + "\n" + ri.getSQL(dc) + extraDebugging);
			else
				throw new RuntimeException(e.toString() + extraDebugging);
		}
