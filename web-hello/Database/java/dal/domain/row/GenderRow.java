...
/**
 * Represents a single row of the <code>Gender</code> table.
 ...
 * Instead, one should use the dal.table.GenderTable.EnumeratedItem class
 * so that a join in the database will not be required for static data.
 * <p>
 * Some relevant facts about this class:
 * <ul>
 *   <li> It represents the data that can be stored and retrieved from the <code>Gender</code> table in the database.
 ...
 * </p>
 */
public class GenderRow extends AbstractRow implements dal.domain.Gender
{
	...
	public GenderRow(dal.table.GenderTable table)
	{
		super(table);
	...
	/**
	 * Return true if the row is an instance of the GenderRow class
	...
	public boolean equals(Object row)
	{
		...
		if(! (row instanceof GenderRow)) return false;

		GenderRow compareRow = (GenderRow) row;
	...
	{
		dal.table.GenderTable table = (dal.table.GenderTable) getTable();
