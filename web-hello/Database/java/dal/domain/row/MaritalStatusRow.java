...
/**
 * Represents a single row of the <code>Marital_Status</code> table.
 ...
 * Instead, one should use the dal.table.MaritalStatusTable.EnumeratedItem class
 * so that a join in the database will not be required for static data.
 * <p>
 * Some relevant facts about this class:
 * <ul>
 *   <li> It represents the data that can be stored and retrieved from the <code>Marital_Status</code> table in the database.
 ...
 * </p>
 */
public class MaritalStatusRow extends AbstractRow implements dal.domain.MaritalStatus
{
	...
	public MaritalStatusRow(dal.table.MaritalStatusTable table)
	{
		super(table);
	...
	/**
	 * Return true if the row is an instance of the MaritalStatusRow class
	...
	public boolean equals(Object row)
	{
		...
		if(! (row instanceof MaritalStatusRow)) return false;

		MaritalStatusRow compareRow = (MaritalStatusRow) row;
	...
	{
		dal.table.MaritalStatusTable table = (dal.table.MaritalStatusTable) getTable();
